...
describe(`${canAccept.name}()`, () => {
  it(`matches file.type with accept MIME types`, () => {
    expect(canAccept({type: 'image/svg+xml'}, [FILE.MIME_TYPE.SVG].join())).toBeTruthy()
    expect(canAccept({type: 'image/png'}, [FILE.MIME_TYPE.PNG, FILE.EXT.JPG].join())).toBeTruthy()
    expect(canAccept({type: 'image/jpeg'}, [FILE.MIME_TYPE.PNG].join())).toBeFalsy()
  })
  it(`matches file.name with accept MIME types`, () => {
    expect(canAccept({name: 'file.svg'}, [FILE.MIME_TYPE.SVG].join())).toBeTruthy()
    expect(canAccept({name: 'file.png'}, [FILE.MIME_TYPE.PNG, FILE.MIME_TYPE.JPG].join())).toBeTruthy()
    expect(canAccept({name: 'file.gif'}, [FILE.MIME_TYPE.PNG, FILE.EXT.JPG].join())).toBeFalsy()
    expect(canAccept({name: 'file.jpg'}, [FILE.MIME_TYPE.JPG].join())).toBeTruthy()
    expect(canAccept({name: 'file.jpeg'}, [FILE.MIME_TYPE.JPG].join())).toBeTruthy()
  })
  it(`matches file.type with accept wildcard MIME types`, () => {
    expect(canAccept({type: 'image/svg+xml'}, ['image/*'].join())).toBeTruthy()
    expect(canAccept({type: 'image/png'}, ['image/*', 'video/*'].join())).toBeTruthy()
    expect(canAccept({type: 'image/jpeg'}, ['video/*'].join())).toBeFalsy()
  })
  it(`matches file.name with accept wildcard MIME types`, () => {
    expect(canAccept({name: 'file.svg'}, ['image/*'].join())).toBeTruthy()
    expect(canAccept({name: 'file.png'}, ['image/*', 'video/*'].join())).toBeTruthy()
    expect(canAccept({name: 'file.jpg'}, ['video/*'].join())).toBeFalsy()
  })
  it(`matches file.name with accept extensions`, () => {
    expect(canAccept({name: 'file.svg'}, [FILE.EXT.SVG].join())).toBeTruthy()
    expect(canAccept({name: 'file.png'}, [FILE.EXT.PNG, FILE.EXT.JPG].join())).toBeTruthy()
    expect(canAccept({name: 'file.jpg'}, [FILE.EXT.PNG].join())).toBeFalsy()
    expect(canAccept({name: 'file.jpg'}, [FILE.EXT.JPEG, FILE.EXT.JPG].join())).toBeTruthy()
  })
})
